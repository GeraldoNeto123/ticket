---
name: split
description: "Fatia o spec em tickets, rodando o portão de modelagem de domínio antes."
...
**O procedimento está em [`../../references/envolver-upstream.md`](../../references/envolver-upstream.md)** — resolver o caminho da versão instalada, ler em vez de invocar, e conferir a forma. Rode-o com `to-tickets`.

**Leia o arquivo agora, mas execute-o só no passo 3.** Ler antes é o que te permite conferir se o portão do passo 2 ainda encaixa no que ele espera receber.

Os pressupostos a conferir: **arestas de bloqueio** por ticket (`Blocked by` / link nativo), um **template** de ticket e a noção de **frontier**.
...
Agora sim, siga o `to-tickets` que você leu, com três acréscimos:
